        ...
        <form action="{{ route('admin.categories.store') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <div style="display: flex; gap: 10px;">
                    ...
                    <input type="text" name="icon" id="icon-input" class="form-control" value="{{ old('icon', 'fa-compass') }}" placeholder="e.g. fa-mountain">
                </div>
            ...
            <div class="form-group">
                ...
                <div style="display: flex; gap: 10px; align-items: center;">
                    <input type="color" name="color" id="color-picker" value="{{ old('color', '#2EC4B6') }}" style="width: 45px; height: 45px; border: 1px solid var(--admin-border); border-radius: 8px; cursor: pointer; padding: 2px;">
                    <input type="text" id="color-text" class="form-control" value="{{ old('color', '#2EC4B6') }}" style="width: 120px;" readonly>
                </div>
            </div>
            
            <div class="form-group" style="display: flex; gap: 15px; margin-top: 30px;">
                <button type="submit" class="btn btn-primary" style="padding: 12px 30px;">Save Category</button>
...
<script>
    document.getElementById('icon-input').addEventListener('input', function() {
        document.getElementById('icon-preview').innerHTML = '<i class="fa-solid ' + this.value + '"></i>';
    });
